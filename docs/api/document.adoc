...
Access, update and delete documents via this RESTful API

== Authorization

In order to access the Document REST API you must first create an <<../admin/apiKeys#, API-Key>>..

The API key must then be passed as a header together with the request, as described in the examples below. For an API-key with the value `mySecretKey`, the Authorization header should look like this:

  Authorization: Explorer-Api-Key mySecretKey

== Endpoint

The API is available on the following URL by default:

  localhost:8080/webapp/com.enonic.app.explorer/documents/api/v1


=== Vhost
When deploying to production, you will most likely explose the API via a vhost. Here is an example of what this might look like:
...
mapping.collectionsApi.target = /webapp/com.enonic.app.explorer/documents/api/v1
----

Using the vhost above, the API will be availabe on `docs.mycompany.com`
...
.Example request from XP controller
[source,js]
----
...
.Example request using an XP controller
[source,js]
----
import {request} from '/lib/http-client';
...
.Requested from an XP controller
[source,js]
----
import {request} from '/lib/http-client';
